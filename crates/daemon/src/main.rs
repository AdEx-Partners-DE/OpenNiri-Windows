...
#[command(name = "openniri", about = "OpenNiri tiling window manager daemon")]
pub struct Args {
...
/// IPC responder timeout for recovery commands that can outlive normal paths.
const IPC_RECOVERY_RESPONSE_TIMEOUT: Duration = Duration::from_secs(20);
/// Max time allowed to enqueue an IPC command into the daemon event queue.
const IPC_ENQUEUE_TIMEOUT: Duration = Duration::from_millis(750);
...
/// Number of recovery passes to run during shutdown when workers timed out.
const SHUTDOWN_RECOVERY_RETRY_ATTEMPTS: usize = 3;
/// Delay between additional recovery passes during shutdown.
...
fn layout_apply_timeout_message(timeout: Duration) -> String {
    format!(
        "Layout application timed out after {} ms; tiling auto-paused to keep the daemon responsive. Resolve blocked Win32 placement and run `openniri-cli toggle-pause` to resume.",
        timeout.as_millis()
    )
}
...
fn response_for_ipc_wait_failure(cmd: &IpcCommand, timed_out: bool) -> IpcResponse {
    if matches!(cmd, IpcCommand::Stop | IpcCommand::PanicRevert) {
        if timed_out {
            IpcResponse::error(
                "Shutdown requested but confirmation timed out; daemon may still be stopping",
            )
        } else {
            IpcResponse::error("Shutdown requested but daemon closed before confirming completion")
        }
    } else if timed_out {
...
fn ipc_response_timeout_for_command(cmd: &IpcCommand) -> Duration {
    match cmd {
...
struct AppState {
    ...
    #[cfg(test)]
    injected_window_info: HashMap<u64, openniri_platform_win32::WindowInfo>,
...
impl AppState {
    ...
    fn lookup_window_info(&self, hwnd: u64) -> Option<openniri_platform_win32::WindowInfo> {
        #[cfg(test)]
        {
        ...
        I: IntoIterator<Item = u64>,
    {
        ...
        let deadline = now
            .checked_add(self.layout_apply_timeout)
            .and_then(|t| t.checked_add(MOVED_OR_RESIZED_SUPPRESSION_WINDOW))
            .unwrap_or_else(|| now + MOVED_OR_RESIZED_SUPPRESSION_WINDOW);
    ...
    fn reap_finished_pending_apply_workers(&mut self) {
    ...
    fn apply_layout(&mut self) -> Result<()> {
        ...
        let spawn_result = std::thread::Builder::new()
            .name("openniri-apply-layout".to_string())
            .spawn(move || {
                let should_cancel = || {
                ...
                let result =
                    openniri_platform_win32::apply_placements(&all_placements, &platform_config)
    ...
    fn sync_foreground_window(&mut self) {
        ...
        if let Some(hwnd) = focused_hwnd {
            ...
            if self.config.appearance.active_border {
                ...
                if let Ok(color) =
                    u32::from_str_radix(&self.config.appearance.active_border_color, 16)
                {
    ...
    fn handle_window_event(&mut self, event: WindowEvent) {
        ...
        let window_id = match &event {
            ...
            | WindowEvent::MovedOrResized(id) => Some(*id),
        ...
        if let Some(wid) = window_id {
            if !matches!(event, WindowEvent::Destroyed(_))
                ...
                && !openniri_platform_win32::is_valid_window(wid)
            {
        ...
        match event {
            ...
            WindowEvent::Focused(hwnd) => {
                ...
                if let Some(monitor_id) = self.find_window_workspace(hwnd) {
                    ...
                    // Always track the OS-foreground window — including floating
                    // windows — so that ToggleFloating can reliably detect and
                    // unfloat the currently focused floating window.
                    self.previous_focused_hwnd = Some(hwnd);
            ...
            WindowEvent::Minimized(hwnd) => {
                if let Some(monitor_id) = self.find_window_workspace(hwnd) {
                    if let Some(workspace) = self.workspaces.get_mut(&monitor_id) {
                        if workspace.mark_minimized(hwnd) {
            ...
            WindowEvent::MovedOrResized(hwnd) => {
                ...
                if self.applying_layout || self.should_suppress_moved_or_resized(hwnd) {
                    debug!(
                        "Suppressing MovedOrResized during/after apply_layout for window {}",
                        hwnd
                    );
                    return;
                }
                // If the window is managed (tiled), snap it back to its layout position.
                // Floating and unmanaged windows are left alone.
                if let Some(monitor_id) = self.find_window_workspace(hwnd) {
...
fn run_visibility_recovery_pass(
    managed_window_ids: &[u64],
    hide_strategy: openniri_platform_win32::HideStrategy,
    mode: ShutdownMode,
) {
    let discovered_window_ids = match enumerate_windows() {
        ...
        Err(e) => {
            warn!(
                "Failed to enumerate windows during {} cleanup: {}",
                mode.label(),
                e
            );
            Vec::new()
    ...
    if hide_strategy == openniri_platform_win32::HideStrategy::MoveOffScreen {
...
async fn run_shutdown_cleanup(state: &Arc<Mutex<AppState>>, mode: ShutdownMode) {
    ...
    let (managed_window_ids, hide_strategy, pending_apply_workers, apply_timeout) = {
        ...
        (
            ...
            state.platform_config.hide_strategy,
    ...
    for handle in pending_apply_workers {
        if !join_with_timeout(handle, apply_timeout) {
            timed_out_workers = timed_out_workers.saturating_add(1);
            warn!(
    ...
    run_visibility_recovery_pass(&managed_window_ids, hide_strategy, mode);

    if timed_out_workers > 0 {
        for attempt in 2..=SHUTDOWN_RECOVERY_RETRY_ATTEMPTS {
            tokio::time::sleep(SHUTDOWN_RECOVERY_RETRY_DELAY).await;
            warn!(
                "Running {} cleanup recovery pass {}/{} after {} timed-out apply workers",
                mode.label(),
                attempt,
...
async fn run_ipc_server(event_tx: mpsc::Sender<DaemonEvent>) {
    ...
    loop {
        ...
        tokio::spawn(async move {
            let permit = match connection_limit.acquire_owned().await {
                Ok(permit) => permit,
                Err(_) => {
...
async fn handle_client(
    ...
    // Send the command to the event loop with a bounded enqueue timeout.
    let enqueue_result = tokio::time::timeout(
        IPC_ENQUEUE_TIMEOUT,
        event_tx.send(DaemonEvent::IpcCommand {
            cmd,
            responder: resp_tx,
        }),
    )
    .await;
    if matches!(enqueue_result, Ok(Err(_))) {
    ...
    if enqueue_result.is_err() {
        let response = IpcResponse::error("Daemon event queue is busy; try the command again");
        let response_json = match serde_json::to_string(&response) {
            Ok(json) => json + "\n",
            Err(e) => {
                warn!("Failed to serialize IPC error response: {}", e);
    ...
    let response_timeout = ipc_response_timeout_for_command(&response_cmd);
...
fn spawn_forwarding_thread<T: Send + 'static>(
    ...
    let builder_name = thread_name.clone();
    let error_name = thread_name.clone();
    std::thread::Builder::new()
        .name(builder_name)
        .spawn(move || {
            let mut dropped_events = 0u64;
            while let Ok(event) = receiver.recv() {
                match sender.try_send(map_fn(event)) {
                    Ok(()) => {}
                    ...
                    Err(mpsc::error::TrySendError::Full(_)) => {
                        ...
                        if dropped_events == 1 || dropped_events.is_multiple_of(100) {
                            warn!(
                                "{} dropping events because daemon queue is full (dropped={})",
                                thread_name, dropped_events
                            );
                        }
                    }
                }
        ...
        .map_err(|e| anyhow::anyhow!("Failed to spawn {} thread: {}", error_name, e))
}

/// Join a thread with a timeout. Returns true if the thread joined within the deadline,
/// false if it timed out. Uses a watcher thread pattern since std::thread::JoinHandle
/// does not support timed joins.
...
pub fn format_startup_banner(info: &StartupInfo) -> String {
    ...
    writeln!(out).unwrap();
    writeln!(out, "OpenNiri Windows v{}", info.version).unwrap();
...
fn format_crash_report(info: &std::panic::PanicHookInfo<'_>) -> String {
    ...
    writeln!(report, "OpenNiri Crash Report").unwrap();
...
async fn main() -> Result<()> {
    ...
    std::panic::set_hook(Box::new(move |info| {
            ...
            Ok(windows) => {
                ...
                match restore_windows_moved_offscreen(&window_ids) {
                    Ok(restored) => {
                        if restored > 0 {
                            eprintln!(
                                "[openniri] Restored {} MoveOffScreen window(s) in panic recovery",
                                restored
                            );
                        }
                    }
                    Err(e) => {
                        eprintln!("[openniri] MoveOffScreen panic recovery failed: {}", e);
                    }
                }
            }
            Err(e) => {
                eprintln!(
                    "[openniri] Failed to enumerate windows for MoveOffScreen recovery: {}",
                    e
                );
            }
        }
        ...
        let ts = std::time::SystemTime::now()
            ...
            .map(|d| d.as_secs())
            .unwrap_or(0);
        let crash_path = std::env::temp_dir().join(format!("openniri-crash-{}.txt", ts));
        let report = format_crash_report(info);
        if let Err(e) = std::fs::write(&crash_path, &report) {
            eprintln!("[openniri] Failed to write crash report: {}", e);
        } else {
            eprintln!(
                "[openniri] Crash report written to: {}",
                crash_path.display()
            );
        }
    ...
    if check_already_running().await {
        eprintln!("Error: Another openniri-daemon instance is already running.");
        eprintln!("Use 'openniri-cli status' to check the running instance.");
        error!(
            "Another openniri-daemon instance is already running (active pipe candidates: {})",
            ipc_pipe_names.join(", ")
        );
        std::process::exit(1);
    }
    ...
    info!("Enumerating windows...");
    {
        ...
        // Skip monitors whose scroll offset was restored from saved state.
        for (monitor_id, workspace) in state.workspaces.iter_mut() {
            if workspace.column_count() > 0 && !restored_monitors.contains(monitor_id) {
        ...
        match event {
            DaemonEvent::IpcCommand { cmd, responder } => {
                if matches!(&cmd, IpcCommand::Stop) {
                    ...
                    if responder.send(IpcResponse::Ok).is_err() {
                        debug!("Client disconnected before receiving stop response");
                    }
                    break;
                }
                if matches!(&cmd, IpcCommand::PanicRevert) {
                    warn!("IPC: panic_revert requested");
                    run_shutdown_cleanup(&state, ShutdownMode::PanicRevert).await;
                    if responder.send(IpcResponse::Ok).is_err() {
                        debug!("Client disconnected before receiving panic_revert response");
                    }
                    break;
                }
            ...
            DaemonEvent::Hotkey(hotkey_event) => {
                let (should_animate, is_resize, column_rect, hint_duration) =
                    if let Some(cmd) = hotkey_state.mapping.get(&hotkey_event.id) {
            ...
            DaemonEvent::Gesture(gesture_event) => {
                ...
                if let Some(cmd) = config::parse_command(cmd_str) {
                    ...
                    // Start animation timer if needed
                    if should_animate
                        && !animation_running.load(std::sync::atomic::Ordering::SeqCst)
                    {
                        animation_timer_handle = Some(start_animation_timer(
                            event_tx.clone(),
                            animation_running.clone(),
    ...
    for handle in thread_handles {
        ...
        if !join_with_timeout(handle, per_thread) {
...
mod tests {
    ...
    fn test_spawn_forwarding_thread_stops_on_channel_close() {
        ...
        let handle =
            spawn_forwarding_thread("test-close", rx, async_tx, |_| DaemonEvent::AnimationTick)
                .unwrap();
    ...
    #[test]
    fn test_ipc_recovery_response_timeout_is_reasonable() {
        assert!(IPC_RECOVERY_RESPONSE_TIMEOUT.as_secs() >= IPC_RESPONSE_TIMEOUT.as_secs());
        assert!(IPC_RECOVERY_RESPONSE_TIMEOUT.as_secs() <= 120);
    }

    #[test]
    fn test_ipc_enqueue_timeout_is_reasonable() {
        assert!(IPC_ENQUEUE_TIMEOUT.as_millis() >= 100);
        assert!(IPC_ENQUEUE_TIMEOUT.as_secs() <= 5);
    }

    #[test]
    fn test_shutdown_recovery_retry_budget_is_reasonable() {
        assert!((1..=5).contains(&SHUTDOWN_RECOVERY_RETRY_ATTEMPTS));
        assert!(SHUTDOWN_RECOVERY_RETRY_DELAY.as_millis() >= 50);
        assert!(SHUTDOWN_RECOVERY_RETRY_DELAY.as_secs() <= 2);
    }

    #[test]
    fn test_response_for_ipc_wait_failure_shutdown_commands_return_error() {
        ...
        match response_for_ipc_wait_failure(&IpcCommand::PanicRevert, false) {
            ...
            other => panic!(
                "Expected panic_revert responder error response, got {:?}",
                other
            ),
        }
    }
    ...
    fn test_args_parse_no_flags() {
        let args = Args::try_parse_from(["openniri"]).unwrap();
    ...
    fn test_args_parse_safe_mode() {
        let args = Args::try_parse_from(["openniri", "--safe-mode"]).unwrap();
        assert!(args.safe_mode);
        assert!(args.skip_hotkeys());
        assert!(args.skip_cloak());
    }
    ...
    fn test_args_parse_no_hotkeys() {
        let args = Args::try_parse_from(["openniri", "--no-hotkeys"]).unwrap();
        assert!(args.no_hotkeys);
        assert!(!args.no_cloak);
        assert!(!args.safe_mode);
    }
    ...
    fn test_args_parse_no_cloak() {
        let args = Args::try_parse_from(["openniri", "--no-cloak"]).unwrap();
        assert!(args.no_cloak);
        assert!(!args.no_hotkeys);
        assert!(!args.safe_mode);
    }
    ...
    fn make_banner_info() -> StartupInfo {
        StartupInfo {
            ...
            config_path: Some(
                "C:\\Users\\test\\AppData\\Roaming\\openniri\\config\\config.toml".to_string(),
            ),
            config_warnings: vec![],
            log_path: "C:\\Users\\test\\AppData\\Local\\Temp\\openniri-daemon.log".to_string(),
            safe_mode: false,
            no_hotkeys: false,
            no_cloak: false,
        }
    ...
    fn test_startup_banner_typical_values() {
        ...
        assert!(banner.contains("OpenNiri Windows v0.1.0"));
    ...
    fn test_apply_layout_timeout_worker_is_joined_during_shutdown_begin() {
        ...
        for handle in workers {
            assert!(
                join_with_timeout(handle, Duration::from_millis(300)),
    ...
    fn make_test_window_info(hwnd: u64) -> openniri_platform_win32::WindowInfo {
        openniri_platform_win32::WindowInfo {
            hwnd,
    ...
    fn test_check_already_running_with_isolated_pipe() {
        ...
        let pipe_name = format!(r"\\.\pipe\openniri-test-singleton-{}", std::process::id());
