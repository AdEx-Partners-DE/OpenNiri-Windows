...
fn pipe_connect_retry_timeout_message(
    ...
    match (saw_busy, saw_not_found) {
        (true, false) => format!(
            "Timed out after {timeout_ms}ms connecting to daemon IPC pipe: the pipe remained busy (daemon may be starting or shutting down). Run `openniri-cli status` and retry once the daemon is stable."
        ),
        (false, true) => format!(
            "Timed out after {timeout_ms}ms connecting to daemon IPC pipe: the pipe was not found (daemon is likely not running). Start it with `openniri-cli run`."
        ),
        (true, true) => format!(
            "Timed out after {timeout_ms}ms connecting to daemon IPC pipe: observed both busy and not-found states (daemon may be transitioning startup/shutdown). Run `openniri-cli status`, then retry."
        ),
        (false, false) => format!(
            "Timed out after {timeout_ms}ms connecting to daemon IPC pipe. Run `openniri-cli status` to check daemon health."
        ),
    }
}
...
fn pipe_connect_not_found_fast_fail_message(cutoff: Duration) -> String {
    format!(
        "Daemon IPC pipe was not found after {}ms. Daemon is likely not running. Start it with `openniri-cli run`.",
        cutoff.as_millis()
    )
}
...
fn panic_revert_not_running_message() -> &'static str {
    "Daemon is not running. Local emergency visibility restore was executed (same action as `openniri-cli emergency-uncloak`)."
}

fn panic_revert_unconfirmed_message() -> &'static str {
    "Daemon disconnected before confirming panic-revert completion. Local emergency visibility restore was executed. Verify windows are visible, run 'openniri-cli status' (it should fail if daemon exited), and run 'openniri-cli stop' if the daemon still responds."
}

fn panic_revert_timeout_recovery_message() -> &'static str {
    "Timed out waiting for panic-revert response. Local emergency visibility restore was executed. Run 'openniri-cli status' to confirm daemon shutdown."
}

fn stop_timeout_recovery_message() -> &'static str {
    "Timed out waiting for daemon stop confirmation. Run 'openniri-cli status' to verify shutdown; if windows remain hidden, run 'openniri-cli panic-revert' or `openniri-cli emergency-uncloak`."
}

fn apply_not_running_message() -> &'static str {
    "Daemon is not running. Start it with `openniri-cli run` (or `openniri-cli run --safe-mode`) before applying layout."
}

fn apply_timeout_recovery_message() -> &'static str {
    "Timed out waiting for `apply` response. If desktop control degrades, run `openniri-cli panic-revert` first, or run `openniri-cli emergency-uncloak` from any reachable terminal."
}

fn apply_unconfirmed_recovery_message() -> &'static str {
    "Apply completion was not confirmed. Local emergency visibility restore was executed. Verify windows are visible, then run `openniri-cli status` before retrying."
}

fn apply_error_response_recovery_message() -> &'static str {
    "Daemon returned a non-success apply response. Local emergency visibility restore was executed. Verify windows are visible, then run `openniri-cli status` before retrying."
}

fn stop_error_response_recovery_message() -> &'static str {
    "Daemon returned a non-success stop response. Local emergency visibility restore was executed. Treat shutdown as unconfirmed and run `openniri-cli status`."
}

fn panic_revert_error_response_recovery_message() -> &'static str {
    "Daemon returned a non-success panic-revert response. Local emergency visibility restore was executed. Verify windows are visible and run `openniri-cli status`."
}
...
fn stop_unconfirmed_message() -> &'static str {
    "Daemon stop was not confirmed. Treat this as unconfirmed shutdown: run 'openniri-cli status', and if windows remain hidden run 'openniri-cli panic-revert' or `openniri-cli emergency-uncloak`."
}
...
async fn handle_status() -> Result<()> {
    if !probe_daemon_running()? {
        anyhow::bail!("Daemon is not running. Start it with `openniri-cli run`.");
    }
    ...
    match probe_daemon_running() {
        Ok(true) => {
            ...
            match send_command(IpcCommand::QueryStatus).await {
                ...
                Err(e) => CheckResult::Fail(format!(
                    "Daemon IPC is reachable but status query failed: {}. Run `openniri-cli panic-revert` (or `openniri-cli emergency-uncloak`) before retrying.",
                    e
                )),
            }
            .print();
        ...
        Err(e) => {
            CheckResult::Warn(format!(
                "Unable to probe daemon state: {}. If the daemon may be running, try 'openniri-cli status'.",
                e
            ))
            .print();
        }
...
mod tests {
    ...
    fn test_cli_alias_recover_parses_to_panic_revert() {
        let cli = Cli::try_parse_from(["openniri-cli", "recover"]).expect("alias should parse");
        assert!(matches!(cli.command, Commands::PanicRevert));
    }

    #[test]
    fn test_cli_alias_pause_parses_to_toggle_pause() {
        let cli = Cli::try_parse_from(["openniri-cli", "pause"]).expect("alias should parse");
    ...
    fn test_cli_alias_restore_windows_parses_to_emergency_uncloak() {
        let cli =
            Cli::try_parse_from(["openniri-cli", "restore-windows"]).expect("alias should parse");
    ...
    fn test_pipe_connect_retry_timeout_message_not_found_only() {
        ...
        assert!(message.contains("openniri-cli run"));
    }
    ...
    fn test_error_chain_indicates_pipe_not_found_timeout_true() {
        let err = anyhow::anyhow!(
            "Timed out after 1000ms connecting to daemon IPC pipe: the pipe was not found (daemon is likely not running). Start it with `openniri-cli run`."
        );
    ...
    fn test_panic_revert_not_running_message_is_actionable() {
        ...
        assert!(message.contains("openniri-cli emergency-uncloak"));
    }
    ...
    fn test_stop_timeout_recovery_message_is_actionable() {
        ...
        assert!(message.contains("Timed out"));
        assert!(message.contains("openniri-cli status"));
        assert!(message.contains("openniri-cli panic-revert"));
        assert!(message.contains("openniri-cli emergency-uncloak"));
    }
