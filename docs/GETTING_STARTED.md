...
If `openniri-cli` or `openniri` is not recognized, run binaries directly from the build output:

```powershell
.\target\release\openniri-cli.exe status
```

You can also add the binary folder to your `PATH`.
...
If the tray icon is not visible, open the Windows "hidden icons" panel and pin OpenNiri so recovery actions stay accessible.
...
4. **Need to temporarily neutralize tiling without exit**: Run `openniri-cli toggle-pause` (alias: `openniri-cli pause`).
5. **Need to recover from a stuck desktop**: Run `openniri-cli panic-revert` first (alias: `openniri-cli recover`; it now runs a local visibility restore automatically if daemon IPC is unavailable), confirm shutdown with `openniri-cli status` (must fail: timeout OR "Daemon is not running..."), then retry with `openniri-cli run --safe-mode` if needed. `openniri-cli stop` now also waits for shutdown confirmation and triggers the same local restore safety net if confirmation is ambiguous. `stop`/`panic-revert` control OpenNiri (`openniri.exe`) only; they should not close your other apps by themselves. You can run `openniri-cli emergency-uncloak` directly at any time (alias: `openniri-cli restore-windows`) or use hotkey `Win+Ctrl+Escape` for emergency restore + panic-revert. If no terminal is reachable, use tray **Emergency: Uncloak All Windows** then **Exit**. Use Task Manager kill/sign-out/reboot only as last resort (can interrupt active work).
