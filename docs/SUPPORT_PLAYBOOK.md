...
## Critical: Desktop Becomes Unusable After `apply` / `reload`
...
1. Run panic revert first:
   - `openniri-cli panic-revert` (sends `panic_revert` to restore/uncloak managed windows and exit cleanly)
2. Verify daemon is down before any rerun:
   - `openniri-cli status` should timeout/fail (no daemon response)
3. If CLI is blocked but the tray is reachable, use tray action **Emergency: Uncloak All Windows**, then click **Exit**.
...
- After step 2, `status` confirms no daemon is active before any rerun.
...
2. Re-run `openniri-cli status` from any recovered terminal to confirm no daemon response.
...
5. Safe mode fallback: stop the daemon, confirm shutdown with `openniri-cli status` (no response), then restart in safe mode via `openniri-cli run --safe-mode` and control via CLI commands.
...
**Steps**:
1. Run panic revert first: `openniri-cli panic-revert`.
2. Confirm daemon shutdown: `openniri-cli status` should timeout/fail.
3. If the daemon still responds, stop it explicitly: `openniri-cli stop`, then rerun `openniri-cli status`.
4. If the daemon crashed, run Alt+Tab to check if windows are still there but cloaked.
5. Disable cloaking in config:
   ```toml
   [appearance]
   use_cloaking = false
   ```
6. Restart: `openniri-cli run`
7. If windows are still missing, they may have been moved off-screen. Use Win+Shift+Left/Right (Windows snap) to bring them back.
...
5. Reset only if needed:
   - Backup first: `Copy-Item "$env:APPDATA\\openniri\\config\\config.toml" "$env:APPDATA\\openniri\\config\\config.toml.bak"`
   - Then reset defaults: `openniri-cli config reset`
...
5. Restart the daemon with stop check:
   - `openniri-cli stop`
   - `openniri-cli status` should timeout/fail
   - `openniri-cli run`
