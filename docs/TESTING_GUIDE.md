...
**Steps**: If a daemon is running, stop it cleanly (`openniri-cli stop`) and confirm shutdown with `openniri-cli status` (must fail before rerun). Open 3+ windows. Run `openniri-cli run`.
...
2. Run `openniri-cli status` and confirm it fails with "Failed to connect..." before any rerun.
...
**Watch for**: Windows left invisible (cloaked). Daemon not responding to stop.
...
2. Run `openniri-cli status` and confirm it fails with "Failed to connect..." before rerun.
...
3. Optionally restart in safe mode after shutdown confirmation: `openniri-cli run --safe-mode`.
