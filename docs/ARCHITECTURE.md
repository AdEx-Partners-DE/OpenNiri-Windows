...
- `openniri-platform-win32` has real Win32 implementations (27 unit tests, 3 hardware-dependent):
  ...
  - `set_foreground_window()` - SetForegroundWindow for actual focus changes
  - `set_active_border()` - Active window border via DWM
...
- `openniri-ipc` provides shared IPC types, including CloseWindow, ToggleFloating, ToggleFullscreen, SetColumnWidth, EqualizeColumnWidths, QueryStatus, and PanicRevert commands.
- `openniri-daemon` runs an async event loop with named pipe IPC server:
  ...
  - Ctrl+C shutdown handling routed into daemon event loop
...
- **Implemented** (validated quality gate: 508 total tests — 504 passed, 4 ignored):
  - Core layout engine
  - IPC protocol crate
  - CLI tool
  ...
  - Touchpad gesture support (low-level mouse hook with wheel accumulation, enabled by default)
  ...
  - SetForegroundWindow for actual focus changes
  - CloseWindow command (Win+Shift+Q)
  - ToggleFloating command (Win+F)
  - ToggleFullscreen command (Win+Shift+F)
