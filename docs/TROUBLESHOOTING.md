...
```
openniri-cli stop
openniri-cli status   # must fail (timeout OR "Daemon is not running...") before any new `run`
```

`openniri-cli stop` targets OpenNiri (`openniri.exe`) only; it should not close your unrelated terminals/apps by itself.

If `stop` prints an **unconfirmed shutdown** warning, the CLI has already executed a local emergency visibility restore as a safety net. Treat shutdown as not confirmed, run the same `status` check, and if windows remain hidden run `openniri-cli panic-revert` before any rerun.
...
1. **If you still have terminal control, freeze tiling first (optional).**

   ```
   openniri-cli toggle-pause
   ```

   Alias: `openniri-cli pause`.

2. **Run panic revert first.**

   ```
   openniri-cli panic-revert
   ```

   This sends `panic_revert` to restore/uncloak managed windows and exit the daemon cleanly. If daemon IPC is unavailable, the CLI now runs a local emergency visibility restore path automatically. Alias: `openniri-cli recover`.

3. **Confirm shutdown before any rerun.**

   ```
   openniri-cli status
   ```

  `status` must fail (timeout OR "Daemon is not running...") before any new `run`.

4. **If terminal input is blocked but tray is reachable, use tray emergency action.** Right-click the OpenNiri tray icon, click **Emergency: Uncloak All Windows**, then click **Exit**.
   If terminal input is available, `openniri-cli emergency-uncloak` is equivalent to the tray emergency action (alias: `openniri-cli restore-windows`).

5. **Attempt focus recovery keys.** Try `Win+Ctrl+Escape` first (emergency visibility restore + panic-revert), then `Win+Ctrl+Shift+B`, `Alt+Esc`, and `Win+Tab` to click the target terminal/editor window.

6. **Use force-kill/sign-out/reboot only as last resort.** If tray + CLI recovery are unavailable, end `openniri.exe` from Task Manager (`Ctrl+Shift+Esc`). Use sign-out/reboot only if the desktop remains unrecoverable. Warn users that these steps can interrupt active shell/file operations.
...
3. Preferred reset path:

   ```
   openniri-cli config reset
   ```

   This restores defaults and creates a backup at `%APPDATA%\openniri\config\config.toml.bak`.

4. Manual fallback (if `config reset` fails, or you want explicit control): rename the config file:

   ```powershell
   Rename-Item "$env:APPDATA\openniri\config\config.toml" "config.toml.bak"
   ```

5. Optionally regenerate a fresh default config:
...
| Named pipe busy | CLI retries with bounded backoff and fast not-running fail paths | `"Timed out..."` / `"Daemon IPC pipe was not found..."` |
