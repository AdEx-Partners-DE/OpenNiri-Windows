...
OpenNiri-Windows is a local desktop window manager. It:
...
- **Communicates via local named pipe** (`\\.\pipe\openniri`) — the CLI and daemon talk over this pipe, which is accessible only to the local user session
...
OpenNiri's attack surface is minimal by design:
...
The IPC pipe (`\\.\pipe\openniri`) uses default Windows named pipe security:
...
OpenNiri collects **no telemetry**, analytics, crash reports, or usage statistics. Nothing is transmitted over the network — the daemon has no networking code at all.
...
|------|----------|---------|
| Config file | `%APPDATA%\openniri\config\config.toml` | User preferences (gaps, hotkeys, window rules) |
| Daemon log | stderr or `%TEMP%\openniri-daemon.log` | Operational messages (window events, errors) |
| Workspace state | `%APPDATA%\openniri\state.json` | Window positions for session restore |
| Crash reports | `%TEMP%\openniri-crash-*.txt` | Panic message, backtrace, version |
...
All `info!`, `debug!`, `warn!`, and `error!` calls across the daemon, config, tray, and platform crates have been audited. Findings:

- Window titles and class names appear at `info` and `debug` levels during window add/create/ignore events. These are inherently user-visible desktop metadata.
- Process executable names appear during window rule evaluation. Also user-visible via Task Manager.
- No credential-like data, environment variables, or file contents are ever logged.
- Named pipe path is a constant string, not sensitive.

---

## Reporting a Vulnerability

If you discover a security vulnerability in OpenNiri-Windows, please report it responsibly:

1. **Do not open a public issue** for security vulnerabilities
2. Email: **stark.burns@adexpartners.com** with subject "OpenNiri Security"
...
During the alpha and beta phases, only the **latest release** receives security fixes. There is no long-term support (LTS) model at this stage. Once the project reaches stable (1.0), the support policy will be revisited.
